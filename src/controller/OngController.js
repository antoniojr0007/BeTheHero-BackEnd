...
module.exports = {
  ...
  async store(request, response) {
    const { name, cnpj, email, whatsapp, city, uf, password } = request.body;
    ...
    // bcrypt.hashSync(password, 10).then(async (hashSync) => {
    //  await connection('ongs').where('email', email).update({
    //    password: hashSync,
    //  });
    // });
    ...
    return response.json({
      ...
      name: ong.name,
      email: config.smtp.host !== 'host_smtp',
    });
  },
  ...
  async show(request, response) {
    ...
    const ong = await connection('ongs')
      .select(['name', 'cnpj,', 'email', 'whatsapp', 'city', 'uf'])
  ...
  async update(request, response) {
    const { name, cnpj, email, whatsapp, city, uf } = request.body;
    ...
    try {
      ...
      await connection('ongs').where('email', request.user.email).update({
        name,
        cnpj,
        email,
        whatsapp,
  ...
  async delete(request, response) {
    ...
    try {
      await connection('ongs').where('id', ong.id).delete();
    } catch (error) {
      return response.status(400).json({
